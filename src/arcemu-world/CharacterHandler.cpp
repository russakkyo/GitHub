...
void WorldSession::FullLogin(Player* plr)
{
...
#ifdef WIN32
	_player->BroadcastMessage("Server: %sMegaCore %s - %s-Windows-%s", MSG_COLOR_WHITE, BUILD_TAG, CONFIG, ARCH);
#else
	_player->BroadcastMessage("Server: %sMegaCore %s - %s-%s", MSG_COLOR_WHITE, BUILD_TAG, PLATFORM_TEXT, ARCH);
#endif

	// Revision
	_player->BroadcastMessage("Revision: %s%s", MSG_COLOR_CYAN, BUILD_HASH_STR);
	// Bugs
	...
	_player->BroadcastMessage("Online Spieler: %s%u |rDurchschnitt: %s%u|r Max Clients: %s%u",
